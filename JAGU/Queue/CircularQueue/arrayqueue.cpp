...
int ArrayQueue<T>::enqueueAQ(Node<T> element) {
	...
	if (this != NULL) {
		if (isArrayQueueFull() == FALSE) {
			this->rear = (this->rear + 1) % this->maxElementCount; // 원형 큐 바뀐 부분
...
Node<T>* ArrayQueue<T>::dequeueAQ() {
	...
	if (this != NULL) {
		if (isArrayQueueEmpty() == FALSE) {
			...
			if (pReturn != NULL) {
				this->front = (this->front + 1) % this->maxElementCount; // 원형 큐 바뀐 부분
...
int ArrayQueue<T>::isArrayQueueFull() {
	...
	if (this != NULL) {
		if (this->currentElementCount == this->maxElementCount) { // 원형 큐 바뀐 부분
			ret = TRUE;
		}
	}
